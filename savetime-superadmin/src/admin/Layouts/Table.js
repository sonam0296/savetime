...
import { HouseOutlined, BusinessOutlined } from '@material-ui/icons';
...
import { edit, editBusinessData, editBusinessDataBusinessData } from '../../redux/actions/actions';
...
const TableContent = () => {
    ...
    const onEdit = (e, item) => {
        dispatch(editBusinessData(item));
        history.push('/admin/businessUpdate')
    }
            ...
            <Grid >
              ...
              {columns.map((column) => (
                <TableCell
                  key={column.id}
                  align={column.align}
                  style={{ minWidth: column.minWidth, border: '2px solid #D50032' }}
                >
                  {column.label}
                </TableCell>
              ))}
