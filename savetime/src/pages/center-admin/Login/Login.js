...
import "../../auth/Center-Admin/signinForAdmin.css"
...
import { setCenterAdminLoginStatus, setCenterManageAnotherCenter, setLoginData, setLoginDataAdmin, setSelectedAdmin, setSelectedLoginCenter, setWorkerLoginStatus } from "../../../redux/actions/actions";
...
function Login() {
  ...
  const adminsarraySelector = useSelector((state) => state.loginDataAdmin);
  const [adminlogins, setAdminlogins] = useState(adminsarraySelector)
  ...
  // useEffect(() => {
  // loginArray=adminsarraySelector;
  ...
  const loginForCenter = async (e) => {
    ...
    const response = await instance.post(`${requests.fetchMangeAnother}?lang=${language}`, bodyApi, {
      headers: {
    ...
    if (response && response.data) {
      ...
      setAdminlogins([...adminlogins,loginData])
      dispatch(setSelectedLoginCenter(loginData))
      dispatch(setLoginData(loginData))
      dispatch(setCenterManageAnotherCenter(true));
      // dispatch(setLoginDataAdmin(response.data))
  ...
  return (
    ...
    <div className="mainPage-container">
      ...
      <FormControl margin="normal" required fullWidth>
          <InputLabel htmlFor="password">{t("Email")}</InputLabel>
          <Input
            name="email"
            type="email"
            autoComplete="email"
        ...
        <FormControl margin="normal" required fullWidth>
          <InputLabel htmlFor="password">{t("Admin Password")}</InputLabel>
          <Input
            name="password"
            type="password"
